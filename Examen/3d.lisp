...
;;;         Es una lista con un vector representando una tupla con las ordenadas y abscisas 
;;;			una bandera representando que es un puente, el 16 o el 17 y una bandera que indica si
;;;			el movimiento fue horizontal ( 0) o vertical (1)  
;;;         Xi: Abscisa inicial. Yi: Ordenada inicial.
;;;			Xo: Abscisa final. Yo: Ordenada final.
;;;			flag
;;;					Para el estado meta solo nos interesa el primer elemento del estado
...
(defun aptitud(estado)
	(let ( (res 0) (Xo (aref *goal* 1)) (Yo (aref *goal* 0)) 
		  (Xa (aref (first estado) 0)) (Ya (aref (first estado) 1)))
...
;;;=======================================================================================
(defun decimal-a-binario-aux (div);;Funcion general para conversión de decimal a binario
	(let ((resto (mod div 2)))
...
;     "el operador tiene estructura : [<etiqueta-humana> <dirección según las manecillas del reloj de 0 al 7>]"  
  ;;(print (get-cell-walls (aref estado 1) (aref estado 0) ))
  (if (valid-state? estado)
   (let* ((Xa (aref (first estado) 1)) (Ya (aref (first estado) 0)) (walls (get-cell-walls Ya Xa)) (mov (first op))
   		(ad (find-adyacencias (first estado))) (ad0 (decimal-a-binario (aref ad 0)))
     	...
     	(and (case mov;;Verificando los movimientos según sus adyacencias
  			(:Arriba   (and (not (= 1 (nth 4 ad2))))  (if (or (= 17 walls) (= 16 walls)) 
  														  (= 1 (first (last estado)))	
  														   T))
  
  			(:Derecha 	(and  (not (= 1 (nth 3 ad0))) (if (or (= 17 walls) (= 16 walls)) 
  														  (= 0 (first (last estado)))	
  														   T)))
  
  			(:Abajo    (and  (not (= 1 (nth 2 ad6))) (if (or (= 17 walls) (= 16 walls)) 
  														  (= 1 (first (last estado)))	
  														   T)))
  
  			(:Izquierda   (and  (not (= 1 (nth 1 ad4)))  (if (or (= 17 walls) (= 16 walls)) 
  														  (= 0 (first (last estado)))	
  														   T)))) 
...
;"El movimiento de no debe salir del mapa  y no se puede caer en una casilla con 4 paredes"
	(let ((Xa (aref (first estado) 1)) (Ya (aref (first estado) 0)) )
		(if (and   (> (get-maze-rows) Ya ) (>  (get-maze-cols) Xa ))
			 (if (and (>= Ya 0) (>= Xa 0))   (not (= 15  (get-cell-walls Ya Xa))))
			 nil)))
;;;=======================================================================================
;;  get-cell-walls-no-error(y x) y es-puesnte?(y,x)
;;	
;;	get-cell-walls-no-error: Obtiene las paredes de  una celda, pero si no es una coordenada valida
;;							 regresa la celda de 4 paredes
;;	es-puesnte?: Predicado para saber si en una coordenada dada hay un puente
;;;=======================================================================================
(defun es-puente? (y x)
	(if (and (>= x 0) (>= y 0) (> (get-maze-rows) Y ) (>  (get-maze-cols) X ))
			(or (= 16 (get-cell-walls y x)) (= 17 (get-cell-walls y x)))
			NIL))
(defun get-cell-walls-no-error (Y X)
	(if (and (>= Y 0) (>= X 0))
		(get-cell-walls X Y)
		15	))
;;;=======================================================================================
...
"Obtiene el descendiente de [estado] al aplicarle  [op]  SIN VALIDACIONES"
	(let* ((Xa (aref (first estado) 1 ) ) (Ya (aref (first estado) 0))  )
		(case (first op)
			(:Arriba 		 	(list (make-array '(2) :initial-contents (list  (+ 1 Ya) Xa)) (es-puente? (+ 1 Ya) Xa) 
																		 (if (es-puente? (+ 1 Ya) Xa) 1 -1)))
			(:Abajo 		 	(list (make-array '(2) :initial-contents (list  (-  Ya 1) Xa)) (es-puente? (-  Ya 1) Xa ) 
																		 (if (es-puente? (-  Ya 1) Xa) 1 -1)))
			(:Derecha 		 	(list (make-array '(2) :initial-contents (list  Ya (-  Xa 1) )) (es-puente? Ya (-  Xa 1) ) 
																		 (if (es-puente? Ya (-  Xa 1)) 0 -1)))
			(:Izquierda 	 	(list (make-array '(2) :initial-contents (list Ya (+  Xa 1) )) (es-puente? Ya (+  Xa 1) ) 
																		 (if (es-puente? Ya (+  Xa 1)) 0 -1))))))
...
;;;=======================================================================================
;; verificar-puentes(estado) y  verificar-puentes-aux  (estado  lista-memoria  acum)
;;        Dado un estado dado, suponiendo que este en su coordenada tiene un puente 
;;		  retorna una lista con el tipo de movimiento realizado sobre esa celda, si no se
;; 		  ha pasado por esa elda se retorna una lista vacia
;;;=======================================================================================
(defun  verificar-puentes (estado )
	(verificar-puentes-aux estado *memory* '()))
(defun  verificar-puentes-aux  (estado  lista-memoria  acum)
"Busca un estado en una lista de nodos que sirve como memoria de intentos previos"  
     (cond ((or (= 2(length acum )) (null  lista-memoria))  acum)
	        ((equalp  (first estado)  (first (second (first  lista-memoria))))  
	        	(setq acum (append acum  (last (second (first  lista-memoria)))  ))
	        	(verificar-puentes-aux  estado  (rest  lista-memoria) acum))  ;;el estado es igual al que se encuentra en el nodo?
		(T  (verificar-puentes-aux  estado  (rest  lista-memoria) acum) ) ))
;;;=======================================================================================
...
"Realiza una búsqueda ciega, por el método especificado y desde un estado inicial hasta un estado meta"
  ...
  (let ((nodo nil)
	  ...
	  (meta-encontrada  nil)
	  (nivel  0))
  	  (setq edo-inicial  (if (es-puente? (aref edo-inicial 1) (aref edo-inicial 0)) 
  	  						(if (= 16 (get-cell-walls (aref edo-inicial 1) (aref edo-inicial 0))) 
  	  							(list edo-inicial T 0)
  	  							(list edo-inicial T 1))
  	  						(list edo-inicial NIL -1)))
      ...
      (loop until  (or  meta-encontrada
	   ...
	   (cond    ((equalp  edo-meta  (first estado))  
